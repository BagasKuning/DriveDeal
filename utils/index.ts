...
export async function fetchCars(filters: filterProps) {
  ...
  const response = await fetch(`https://cars-by-api-ninjas.p.rapidapi.com/v1/cars?make=${manufacturers}&year=${year}&model=${model}&limit=${limit}&fuel_type=${fuel}`, {
    headers: headers
  })
...
export const generateCarImageUrl = (car: CarProps, angle?: string): string => {
  ...
  url.searchParams.append("customer", "hrjavascript-mastery") // api punya yt js mastery
